	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("status").equals("add_produit")) {
			...
			for (Produit_Commande PC : cmd.getProd_Cmd()) {
				if (prod.getId_Produit() == PC.getProduit_cmd().getId_Produit()) {
					duplicate_prod = true;
				}
			}
			dispatcher = request.getRequestDispatcher("EditCommande.jsp");
			if (duplicate_prod) {
		...
		/*
		 * the Objective of this part is to remove a product from a commande
		 */
