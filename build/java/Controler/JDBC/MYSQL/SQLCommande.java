...
public class SQLCommande {
	...
	public static int UpdateCommande(Commande commande) {
		int st = 0;
		try {
			String sql = "UPDATE `Commande` SET `Id_Client`=?,`Id_produit`=?,`Addresse_Cmd`=?,`Date_cmd`=?,`Montant`=?,"
					+ "`Validtion`=?WHERE Id_cmd=?";
			Connection con = SqlConnection.getConnection();
			PreparedStatement PS = con.prepareStatement(sql);
			PS.setInt(1, commande.getId_Client());
			PS.setInt(2, commande.getId_Produit());
			PS.setString(3, commande.getAddresse_Cmd());
			PS.setDate(4, (Date) commande.getDate_cmd());
			PS.setDouble(5, commande.getMontant());
			PS.setBoolean(6, commande.isValidtion());

			PS.setLong(7, commande.getId_cmd());
