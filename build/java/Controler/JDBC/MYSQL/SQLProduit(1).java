...
public class SQLProduit {
	...
	public static int AddProduit(Produit Produit, InputStream img) {
		int st = 0;
		...
		try {
			...
			String sql = "INSERT INTO `produit`(`Desegnation`, `Categorie`, `Prix`, `Quantite`, `TVA`,`IMAGE`,`Prix_Total`) "
					+ "VALUES  (?,?,?,?,?,?,?)";
			...
			PS.setBlob(6, img);
			PS.setDouble(7, Produit.getPrix() * Produit.getQuantite());
			st = PS.executeUpdate();
			conn.close();
