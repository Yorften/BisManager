...
public class SQLFournisseur {
	...
	public static int AddFournisseur(Fournisseur fournisseur) {
		...
		try {
			...
			PS.setInt(6,fournisseur.getTel());
			PS.setInt(7,fournisseur.getFax() );
	...
	public static Fournisseur getFournisseur(int id_fournisseur) {
		...
		try {
			...
			if(RS.next()) {
				...
				fournisseur.setTel(RS.getInt(7));
				fournisseur.setFax(RS.getInt(8)) ;
	...
	try {
		...
		while(RS.next()) {			
			fourn.addProduit(SQLProduit.getProduitById(RS.getInt(2)));
		}			
		con.close();
	} catch (SQLException e) {
		  e.printStackTrace();
