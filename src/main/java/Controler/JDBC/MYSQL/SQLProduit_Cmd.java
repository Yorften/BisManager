	...
	public static int AddProduitCmd(Commande commande) {
		...
		try {
			...
			for (Produit_Commande PC : Prod_cmd) {
				String sql = "INSERT INTO `commande_produit`(`id_cmd`, `id_Produit`, `Quantite_cmd`, `Desenaion`, `categorie`, `Prix`, `TVA`)"
						+ "VALUES  (?,?,?,?,?,?,?)";
				PreparedStatement PS = conn.prepareStatement(sql);
				PS.setInt(1, commande.getId_cmd());
				PS.setInt(2, PC.getProduit_cmd().getId_Produit());
				PS.setInt(3, PC.getQuantite_Cmd());
				PS.setString(4, PC.getProduit_cmd().getDesegnation());
				PS.setString(5, PC.getProduit_cmd().getCategorie());
				PS.setDouble(6, PC.getProduit_cmd().getPrix());
				PS.setDouble(7, PC.getProduit_cmd().getTVA());

				st = PS.executeUpdate();
				// this part is to update the quantity of the product after the command is been added
				if (st > 0) {
					sql = "UPDATE `produit` SET `Quantite`=Quantite-? where id_Produit=?";
					PS = conn.prepareStatement(sql);
					PS.setInt(1, PC.getQuantite_Cmd());
					PS.setInt(2, PC.getProduit_cmd().getId_Produit());
					st = PS.executeUpdate();
				}
				if (st != 1)
